/*
 * SHA-256 support for DCF2 compatibility with decimen-optical-transfer/shared/protocol.ts.
...
private val SHA256_INITIAL_HASH = intArrayOf(
    0x6a09e667,
    0xbb67ae85.toInt(),
    0x3c6ef372,
    0xa54ff53a.toInt(),
...
internal fun sha256(bytes: ByteArray): ByteArray {
    ...
    val hash = SHA256_INITIAL_HASH.copyOf()
    val schedule = IntArray(64)
    for (chunkOffset in padded.indices step 64) {
        for (index in 0 until 16) {
            schedule[index] = padded.readInt32BigEndian(chunkOffset + index * 4)
        }
        ...
        for (index in 0 until 64) {
            ...
            val temporary1 = h + s1 + choose + SHA256_ROUND_CONSTANTS[index] + schedule[index]
    ...
    return ByteArray(32).also { output ->
        hash.forEachIndexed { index, value -> output.writeInt32BigEndian(index * 4, value) }
    }
}

private fun ByteArray.readInt32BigEndian(offset: Int): Int =
    (this[offset].toUByte().toInt() shl 24) or
        (this[offset + 1].toUByte().toInt() shl 16) or
        (this[offset + 2].toUByte().toInt() shl 8) or
        this[offset + 3].toUByte().toInt()
